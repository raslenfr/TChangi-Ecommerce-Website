...
$idCommande = $_GET['id'];
$sqlState = $pdo->prepare('SELECT commande.*,client.ln as "login" FROM commande 
    ...
    <table class="table table-striped table-hover">
        ...
        <?php foreach ($lignesCommandes as $lignesCommande) : ?>
            <tr>
                ...
                <td>Dinar<?php echo $lignesCommande->prix ?> Dinar</td>
                <td>x <?php echo $lignesCommande->quantite ?></td>
                <td><?php echo $lignesCommande->total ?> Dinar</td>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
